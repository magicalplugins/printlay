...
export default function AdminLeads() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setData(res);
      // Auto-select the first item on filter change if nothing's selected
      // (or the previously-selected one is no longer in the list).
  ...
  // Mark "new" leads as "read" automatically when the admin opens them.
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Silent — admin can still manually toggle.
      }
    })();
    return () => {
      cancelled = true;
  ...
  async function setStatus(id: string, status: LeadStatus) {
    try {
      ...
      // If we just archived, advance selection to next item.
      if (status === "archived" && filter !== "archived") {
        const idx = data?.items.findIndex((l) => l.id === id) ?? -1;
        const next =
          data?.items[idx + 1] ?? data?.items[idx - 1] ?? null;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[360px_1fr] gap-4 min-h-[60vh]">
        ...
        <div className="rounded-xl border border-neutral-800 overflow-hidden bg-neutral-950/60">
          {loading && !data ? (
            ...
            <ul className="divide-y divide-neutral-900 max-h-[70vh] overflow-y-auto">
              {data?.items.map((l) => {
